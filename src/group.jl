...
"""
function add!(group::Group, id="domain", glist=[1], gtype="Region"; operation="=", comment=nothing, kwargs...)
    if operation == "=" && id in group.idlist
        error("Identifier $(id) already in use.")
    elseif operation == "+=" && !(id in group.idlist)
        error("Cannot extend non-existing group $(id).")
    end
...
"""
function ElementsOf(group::Group, args...; OnOneSideOf=nothing, OnPositiveSideOf=nothing, Not=nothing, kwargs...)
    return add!(
        group,
        args...;
        gtype="ElementsOf",
...
function FacetsOfTreeIn(group::Group, args...; StartingOn=nothing, kwargs...)
    return add!(group, args...; gtype="FacetsOfTreeIn", StartingOn=StartingOn, kwargs...)
end
