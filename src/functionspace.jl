...
function add!(functionspace::FunctionSpace, name, operation, constraint; comment=nothing, kwargs...)
    item = Dict(
        :name => name,
        :basis_functions => [],
        :global_quantities => [],
...
function add_basis_function!(functionspace::FunctionSpace, name, nameOfCoef, func; Support, Entity, comment=nothing, kwargs...)
    if isempty(functionspace.items)
        error("No FunctionSpace items defined. Call add! first.")
    end
...
# Add a constraint
function add_constraint!(functionspace::FunctionSpace, nameOfCoef, entityType, nameOfConstraint; comment=nothing, kwargs...)
    if isempty(functionspace.items)
        error("No FunctionSpace items defined. Call add! first.")
    end
    item = functionspace.items[end]  # Target the last added item
...
function add_global_quantity!(functionspace::FunctionSpace, name, type; NameOfCoef, comment=nothing, kwargs...)
    if isempty(functionspace.items)
        error("No FunctionSpace items defined. Call add! first.")
    end
    item = functionspace.items[end]  # Target the last added item
            ...
            for c in item[:constraints]
                ...
                if c[:comment] !== nothing
                    if haskey(c[:kwargs], :condition)
                        ...
                        push!(code_lines, "      " * cc * " " * comment(c[:comment], newline=false))
                    end
                else
                    if haskey(c[:kwargs], :condition)
                        push!(code_lines, "    $(c[:kwargs][:condition])")
